...
namespace Canvas.v1.Wrappers
{
    public class AsUser
    {
        private readonly AsUserType _type;
        private readonly string _id;

        public AsUser(AsUserType type, string id)
        {
            _type = type;
            _id = id;
        }

        public Uri AppendQuery(Uri uri)
        {
            var uriBuilder = new UriBuilder(uri.AbsoluteUri);
            var query = HttpUtility.ParseQueryString(uriBuilder.Query);
            query.Add("as_user_id", SpecifyUser());
            uriBuilder.Query = query.ToString();
            return uriBuilder.Uri;
        }

        private string SpecifyUser()
        {
            switch (_type)
            {
                case AsUserType.Canvas:
                    return _id;
                case AsUserType.SIS:
                    return "sis_user_id:" + _id;
                default:
    ...
    public class HttpValueCollection : Collection<HttpValue>
    {
        ...
        {
            foreach (var toRemove in this.Where(x => string.Equals(x.Key, key, StringComparison.OrdinalIgnoreCase)).ToList())
            {
                Remove(toRemove);
            }
        }
